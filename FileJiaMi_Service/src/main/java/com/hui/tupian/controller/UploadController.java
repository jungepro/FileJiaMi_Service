...
public class UploadController {
    ...
    public Result<?> upload(HttpServletRequest request) {
        ...
        String fileName = uuidStr + ".txt";
        String filePath = rootPath + "/" + fileName;//本地绝对路径
        String filePathTmp = rootPath + "/" + uuidStr + "_tmp.png";
        log.info("[文件接口]上传的文件本地绝对路径为->" + filePath);
        f = new File(filePath);
        try {
            file.transferTo(f);
        } catch (IOException e) {
            e.printStackTrace();
            throw new RuntimeException("[文件接口]写文件到磁盘失败");
        }
        jsonObject.put("path", fileName);
        log.info("/uploadImg->上传文件->结束->" + jsonObject.toJSONString());
